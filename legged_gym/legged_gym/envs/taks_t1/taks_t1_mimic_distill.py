...
from .taks_t1_mimic_distill_config import TaksT1MimicPrivCfg, TaksT1MimicStuCfg
...
class TaksT1MimicDistill(HumanoidMimic):
    def __init__(self, cfg: TaksT1MimicPrivCfg, sim_params, physics_engine,
                 sim_device, headless):
        self.cfg = cfg
    ...
    def _get_body_indices(self):
        upper_arm_names = [s for s in self.body_names
                          if self.cfg.asset.upper_arm_name in s]
        lower_arm_names = [s for s in self.body_names
                          if self.cfg.asset.lower_arm_name in s]
        torso_name = [s for s in self.body_names
                      if self.cfg.asset.torso_name in s]
        self.torso_indices = torch.zeros(
            len(torso_name), dtype=torch.long, device=self.device,
            requires_grad=False)
        ...
        self.upper_arm_indices = torch.zeros(
            len(upper_arm_names), dtype=torch.long, device=self.device,
            requires_grad=False)
        ...
        self.lower_arm_indices = torch.zeros(
            len(lower_arm_names), dtype=torch.long, device=self.device,
            requires_grad=False)
        ...
        knee_names = [s for s in self.body_names
                      if self.cfg.asset.shank_name in s]
    ...
    def _init_buffers(self):
        ...
        self.obs_history_buf = torch.zeros(
            (self.num_envs, self.cfg.env.history_len, self.cfg.env.n_obs_single),
            device=self.device)
        self.privileged_obs_history_buf = torch.zeros(
            (self.num_envs, self.cfg.env.history_len,
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_scale_vec[:, ang_vel_dim+imu_dim:ang_vel_dim+imu_dim+self.num_dof] = \
            self.cfg.noise.noise_scales.dof_pos
        noise_scale_vec[:, ang_vel_dim+imu_dim+self.num_dof:
                        ang_vel_dim+imu_dim+2*self.num_dof] = \
    ...
    def _get_mimic_obs(self):
        ...
        root_rot = root_rot.reshape(
            self.num_envs, num_steps, root_rot.shape[-1])
        root_ang_vel = root_ang_vel.reshape(
            self.num_envs, num_steps, root_ang_vel.shape[-1])
        dof_pos = dof_pos.reshape(
            self.num_envs, num_steps, dof_pos.shape[-1])
        dof_vel = dof_vel.reshape(
            self.num_envs, num_steps, dof_vel.shape[-1])
        root_vel_local = root_vel_local.reshape(
            self.num_envs, num_steps, root_vel_local.shape[-1])
        root_ang_vel_local = root_ang_vel_local.reshape(
            self.num_envs, num_steps, root_ang_vel_local.shape[-1])
        root_pos_delta_local = root_pos_delta_local.reshape(
            self.num_envs, num_steps, root_pos_delta_local.shape[-1])
        root_rot_delta_local = root_rot_delta_local.reshape(
            self.num_envs, num_steps, root_rot_delta_local.shape[-1])
        ...
        return (priv_mimic_obs_buf.reshape(self.num_envs, -1),
